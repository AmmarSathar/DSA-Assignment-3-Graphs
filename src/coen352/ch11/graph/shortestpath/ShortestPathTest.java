...
class ShortestPathTest {
		...
		static StringBuffer out,path; // StringBuffer is used as the recording for information
		...
		static void Dijkstra(Graph G, int s, int[] D) {
		  ...
		  for (int i=0; i<G.n(); i++) {  // Process the vertices
			...
			int v = minVertex(G, D);  //linear searching for the minimal distance in the array;   // Find next-closest vertex
		    //vertex v is now on the shortest path from the starting nodes to v; 
		...
		static void DijkstraMinHeap(Graph G, int s, int[] D) {
		  ...
		  public void testSimpleShortestPath()
				  throws IOException {
				    ...
				    assertEquals("0 5 3 10 18 ", out.toString());
				    assertEquals("0-> 2-> 1-> 3-> 4->", path.toString());
